...
Step 2 & 3: Making a ride_length column as well as transforming timestamp into weekdays,dates and time 
...
SELECT 
 ...
 -- Creates the length of a rides as a column in minutes
 
 FORMAT_DATE('%A',started_at) AS start_day,FORMAT_DATE('%A',ended_at) AS end_day,
 
 -- Formats timestamp into weekdays in name (example "Wednesday") and renames column
...
-- Fully combines the other querries
...
-- At the end of querry used bigquerrys 'save as' function to create a table of the unioned data tables
-- Repeated this twice to make other 2 data chunks


Step 4: Union of temp tables into master data set
----------------------------------------------------------------------------------------------------------
SELECT *
...
-- Utilized bigquerry's 'save as' function to create new table named master_table_oct2021_2022

Step 5: Querring to Find negetative ride_lengths 
...
-- Finds all rows containting rides that are under 0 minutes long

DELETE
FROM `buoyant-genre-370301.Cyclistic_oct2021_oct2022.master_table_oct2021_2022`
WHERE ride_length < 0

-- Only deletes querried results, which are all found negative ride values

Step 7: Data aggregation for Data visualization 
-----------------------------------------------------------------------------------------------------------

-- Counts number of trips by weekday with member type

SELECT member_casual, COUNT(ride_length) as trips, start_day
FROM `buoyant-genre-370301.Cyclistic_oct2021_oct2022.master_table_oct2021_2022` 
GROUP BY member_casual, start_day

-- Counts number of trips and Average ride_lengths for each bike type with member type
...
-- This querry is not uploaded to Tableau, visualization results are retrieved from the JOINS of trips per weekday view and the station usage view

-- Counts number of trips by date with member type
...
-- Counts number of trips made before noon and after noon
...
-- All  querries above in step 7 are saved as a view, then exported to Google Sheets to be later imported into Tableau for visualization
